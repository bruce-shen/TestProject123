...
- (void)layoutSubviews {
	...
	UIImage *background = [UIImage imageNamed:@"TableCell.png"];
	((UIImageView*) self.backgroundView).image = background;
	self.backgroundColor = [[[UIColor alloc] initWithPatternImage:background] autorelease];
	...
	if (_titleLabel.text.length) {
		_titleLabel.frame = CGRectMake(20, 10, 250, 20);
...
//	}
	...
	if (_timestampLabel.text.length) {
		...
		_timestampLabel.left = 20;
		_timestampLabel.top = 35;
		//_timestampLabel.font = [UIFont fontWithName:@"Helvectica" size:7];
